...
test("server bounds launch coordinator shutdown by the shutdown grace period", { timeout: 2_000 }, async () => {
  let registryCloseCalls = 0;
  const registry = {
    revision: 0,
    events: new AgentEventBus(),
    async refresh() { return []; },
    async close() { registryCloseCalls += 1; },
  };
  ...
  const server = createAgentServer(registry, {
    host: "127.0.0.1", port: 0, refreshMs: 60_000, apiToken: API_TOKEN,
    shutdownGraceMs: 5, launchCoordinator,
  });
  await server.start();
  ...
  await server.stop();
  assert.ok(Date.now() - began < 1_000);
  assert.equal(registryCloseCalls, 0);
});

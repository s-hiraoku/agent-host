...
export function renderLaunchAgent({ nodePath, cliPath, configPath, logFile, errorLogFile = `${logFile}.error` }) {
  const args = [nodePath, cliPath, "serve", "--config", configPath];
...
export function createMacosServiceController(options = {}) {
  ...
  return {
    async install({ plistPath, nodePath, cliPath, configPath, logFile }) {
      assertSupported();
      if (await isInstalled(plistPath)) throw new Error(`LaunchAgent is already installed: ${plistPath}`);
      await ensureOwnedDirectory(dirname(plistPath));
      await ensurePrivateDirectory(dirname(logFile));
      await writePrivateFileAtomic(
        plistPath,
        renderLaunchAgent({ nodePath, cliPath, configPath, logFile }),
    ...
    async uninstall(plistPath) {
      ...
      if (!contents.includes(`<string>${SERVICE_LABEL}</string>`)) {
        throw new Error(`refusing to remove unmanaged LaunchAgent: ${plistPath}`);
      }

...
- Lifecycle fixtures: clean install, synthetic 0.2.0 -> 0.3.0 update, explicit rollback, uninstall with `~/.agent-host` preservation, stable launcher generation, version/API/config/dashboard reporting, ABA-safe stale install-lock recovery with persistent inode tombstones, fail-closed incomplete locks, competing recovery rejection, and automatic pointer/state restoration after activation failure. Failed restoration retains its durable transaction marker and the next invocation recovers it before proceeding.
...
- Independent review: Adviser identified transactional recovery and stale-lock ABA races. The final owned-directory lock uses permanent nonempty inode quarantines as fencing tombstones and fails closed for incomplete metadata; transaction cleanup failures retain a consistent committed state plus a durable marker for next-run recovery.

## Issue #11 backend release gate — 2026-08-12

- Host CI: https://github.com/s-hiraoku/agent-host/actions/runs/31594311951 passed Node 22, 23, and 24 plus the integrated artifact job. The integrated job builds dashboard commit `f641cb09e144490297fb3fe6759bac42aeb8e799`, verifies and installs the extracted archive, starts it through the stable launcher, and passes live HTTP/SSE conformance.
- Dashboard CI: https://github.com/s-hiraoku/agent-host-dashboard/actions/runs/31591117758 passed build/unit, browser E2E, accessibility, performance, contract, and live conformance at the pinned commit.
- Decision: controlled MVP evaluation candidate; stable daily-driver NO-GO. The backend contracts for file-change approvals, global sort/facets, and stable local project association are implemented on the Issue #11 branch, but the dashboard has not adopted them. Remaining work also requires external state or user activity: merge PRs #19/#20, publish an RC, perform a clean-Mac timed install and reboot, exercise complete real Herdr/Codex workflows, and use the RC in normal daily work.
- Complete criterion-by-criterion evidence, known limitations, and the close-out procedure are in `docs/release-gate.md`. Issue #11 remains open until those gates pass.
